...
class HighlightedTextEdit(QtGui.QTextEdit):
    ...
    def getFontStep(self):
    
        return self._fontStep
    
    def setFontStep(self, step):
    
        self._fontStep = step
    
    fontStep = QtCore.pyqtProperty("int", getFontStep, setFontStep)
    
    @QtCore.pyqtSignature("")
    def decreaseFontSize(self):
    
        font = self.getDisplayFont()
        newSize = font.pointSize() - self._fontStep
        if newSize >= 1.0:
            font.setPointSizeF(newSize)
            self.setDisplayFont(font)
    
    @QtCore.pyqtSignature("")
    def increaseFontSize(self):
    
        font = self.getDisplayFont()
        font.setPointSizeF(font.pointSize() + self._fontStep)
        self.setDisplayFont(font)
