...
são subconjuntos de 2 elementos de $V$. Para evitar ambiguidades de notação, devemos assumir que $V \cap E = \O{}$. Os elementos de $V$ são os vértices
(ou nós ou pontos) e os elementos são suas arestas (ou linhas) \cite{reinhard2010}.
...
``
Pelo que entendi esse paragrafo esta fechando o capitulo. 

Acho que aqui voce pode então fazer um resumo das caracteristicas gerais do processo destacando os requisitos gerais, tipo:

1) Controle de parametro de simulação
2) persistencia de resultados intermediários
3) produzir resultados visuais para analises;
4) etc…..

E ai entraria a tabela 2.1. Sendo que aqui acho que não cabe falar em Criar projeto, Abrir Projeto, ou coisas do genero, e sim requisitos mais gerais.

A partir deles é que voce no proximo capitulo vai transformar em casos de uso.
''
...
``Falta um paragrafo fechando o capítulo indicando que a partir desses requisitos o sistema NAVI será modelado no proximo capitulo.''
...
A etapa de congruência foi modelada mas não foi implementada, devido a sua complexidade e à falta de tempo hábil. Mais informações sobre a congruência podem
